...
static struct vvcam_mode_info_s pimx219_mode_info[] = {
    {
        ...
        .ae_info = {
            ...
            .start_exposure = 3 * 400 * (1 << SENSOR_FIX_FRACBITS),
...
static int imx219_write_reg(struct imx219 *sensor, u16 reg, u8 val)
{
    ...
    u8 au8Buf[3] = {reg >> 8, reg & 0xff, val};
                                 ...
                                 u32 size)
{
    ...
    for (i = 1; i < size; i++)
    {
        ...
        else
        {
            ...
            send_buf_len = 0;
            send_buf[send_buf_len++] = (reg_arry[i].addr >> 8) & 0xff;
            send_buf[send_buf_len++] = reg_arry[i].addr & 0xff;
            send_buf[send_buf_len++] = reg_arry[i].data & 0xff;
        }
    }
...
{
    int ret = 0;
    ret = copy_to_user(pmode, &sensor->cur_mode, sizeof(struct vvcam_mode_info_s));
    if (ret != 0)
        ret = -ENOMEM;
    return ret;
}
...
static int imx219_set_sensor_mode(struct imx219 *sensor, void *pmode)
{
    ...
    for (i = 0; i < ARRAY_SIZE(pimx219_mode_info); i++)
    {
        ...
        {
            memcpy(&sensor->cur_mode, &pimx219_mode_info[i], sizeof(struct vvcam_mode_info_s));
            return 0;
        }
    }
    return -ENXIO;
                              ...
                              void *arg_user)
{
    ...
    case VVSENSORIOC_G_FPS:
        USER_TO_KERNEL(u32);
        // ret = imx219_get_fps(sensor, arg); //imx219 not support
        break;
    case VVSENSORIOC_GET_I2C_DATA:
        USER_TO_KERNEL(struct vvcam_sccb_data_s);
        ret = imx219_i2c_read(sensor, &sensor_reg);
        KERNEL_TO_USER(struct vvcam_sccb_data_s);
        break;
    case VVSENSORIOC_GET_VCMT:
        ret = imx219_get_vcmt(sensor, arg);
        break;
    default:
        pr_err("error: unsupported ioctl command:%d\n", cmd);
        ret = -EINVAL;
        break;
    }
    mutex_unlock(&sensor->lock);
